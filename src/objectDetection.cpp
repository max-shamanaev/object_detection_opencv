...
namespace od
{
    ...
    void ObjectDetection::run(cv::VideoCapture& cap, bool showTimeProfile, bool showFPS)
    {
        ...
        while (cap.read(frame))
        {
            ...
            if (showTimeProfile)
            {
                // Вектор для хранения таймингов по слоям
                // (на данный момент используется только общее время)
    ...
    {
        // На данный момент не выполняется 
        // обработка исключения, выбрасываемого OpenCV,
